...
// through JSON, refusing a key the configuration does not have. It also
// returns the run section of a golangci-lint configuration, which says how the
// packages are loaded, and which the command's own file does not have.
func readConfig(path string) (paircheck.Config, map[string]any, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		return paircheck.Config{}, nil, err
	}
	var document map[string]any
	if err := yaml.Unmarshal(content, &document); err != nil {
		return paircheck.Config{}, nil, fmt.Errorf("%s: %w", path, err)
	}
	document, err = folded(document)
	if err != nil {
		return paircheck.Config{}, nil, fmt.Errorf("%s: %w", path, err)
	}
	settings, section, err := settingsOf(document)
	if err != nil {
		return paircheck.Config{}, nil, fmt.Errorf("%s: %w", path, err)
	}
...
// configuration, native or as a module plugin, and the run section, which
// only a golangci-lint configuration has.
func settingsOf(document map[string]any) (any, map[string]any, error) {
